...
Este documento detalha o planejamento funcional e tecnico resumido no README.md.

## Relacao com o README
- O README apresenta o contexto geral e o resumo por sprint.
- Este backlog traz o detalhamento completo das historias, epics e planejamento.

## Como usar este backlog
- Fonte unica de planejamento: este arquivo.
- Execucao: issues e tarefas do repositorio.
- Acompanhamento de sprint: milestones.
- Visao de fluxo (To Do, In Progress, In Review, Done): board do projeto.
- Evidencias e validacao: pasta docs/evidencias.

## Escala
- Prioridade: Highest, High, Medium, Low
- Story Points: Fibonacci (1, 2, 3, 5, 8, 13)

### Mapeamento recomendado para labels no GitHub

| Campo | Valor no backlog | Label sugerida no GitHub |
|---|---|---|
| Prioridade | Highest | priority:p0 |
| Prioridade | High | priority:p1 |
| Prioridade | Medium | priority:p2 |
| Prioridade | Low | priority:p3 |
| Tipo de item | User story | type:story |

### Mapeamento de Epic para label no GitHub

| Epic | Label sugerida |
|---|---|
| EPIC-01 Autenticacao e Perfil | epic:auth |
| EPIC-02 Receitas e Conteudo | epic:recipes |
| EPIC-03 Planejamento e Lista de Compras | epic:planning |
| EPIC-04 Inteligencia e Recomendacoes | epic:intelligence |
| EPIC-05 Experiencia de Preparo | epic:cooking-experience |
| EPIC-06 Social, Comunidade e Compartilhamento | epic:social |
| EPIC-07 Offline e Sincronizacao | epic:offline-sync |
| EPIC-08 Arquitetura, API e Qualidade | epic:platform-quality |
| EPIC-09 Acessibilidade e Modos de Uso | epic:accessibility |
| EPIC-10 Analytics e Crescimento | epic:analytics-growth |

### Milestones recomendados no GitHub

| Milestone | Escopo |
|---|---|
| Sprint 1 | PB-34, PB-35, PB-37, PB-39, PB-01, PB-02, PB-03, PB-04, PB-05, PB-06, PB-07, PB-08, PB-09, PB-41 |
| Sprint 2 | PB-13, PB-14, PB-15, PB-16, PB-17, PB-18, PB-31, PB-32, PB-33, PB-36 |
| Sprint 3 | PB-10, PB-11, PB-12, PB-19, PB-20, PB-21, PB-22, PB-23, PB-24, PB-25, PB-26, PB-27, PB-28, PB-29, PB-30, PB-38, PB-40 |

### Padrao de titulo de issue no GitHub
- PB-XX - resumo curto da historia
...
## Backlog Priorizado (fonte para GitHub Issues)
...
| PB-37 | EPIC-08 | Arquitetura modular por features com injecao de dependencia e fluxo reativo | Highest | 8 |
...
## Verificacao de coerencia com os requisitos

Status da revisao:
- Requisitos funcionais cobertos: 30 de 30.
- Requisitos nao funcionais cobertos: 10 de 10.
- Ajuste aplicado: PB-37 passou a explicitar injecao de dependencia e fluxo reativo para cobrir o requisito arquitetural equivalente ao item de MVVM/LiveData/Flow.
- Ajuste aplicado: removida a estrutura pos-MVP e redistribuido 100% do backlog em 3 sprints, conforme regra de entrega.

### Mapeamento rapido de requisitos nao funcionais

| RNF | Cobertura no backlog |
|---|---|
| RNF-01 API REST Node + Express + MongoDB | PB-34 |
| RNF-02 API versionada e Swagger | PB-35 |
| RNF-03 Tempo real com Firebase ou WebSockets | PB-16, PB-36 |
| RNF-04 Offline com cache e sincronizacao | PB-31, PB-32, PB-33 |
| RNF-05 Testes de interface | PB-38 |
| RNF-06 Arquitetura com DI e fluxo reativo | PB-37 |
| RNF-07 ML local para reconhecimento por foto | PB-21 |
| RNF-08 Otimizacao para telas diferentes | PB-39 |
| RNF-09 Analytics com consentimento e anonimizado | PB-41 |
| RNF-10 Modularizacao por features | PB-37 |

## Plano obrigatorio em 3 sprints (100% dos requisitos)

Premissas:
- Todas as historias PB-01 a PB-41 devem ser entregues dentro das 3 sprints.
- Pontuacao total do backlog: 288 pontos.
- Distribuicao final: Sprint 1 = 86 pontos, Sprint 2 = 85 pontos, Sprint 3 = 117 pontos.

## User Stories por Sprint (entrega integral)

### Sprint 1 - Fundacao, autenticacao e base de conteudo (86 pontos)

Objetivo da sprint:
- Construir fundacao tecnica completa e liberar o ciclo principal de autenticacao + cadastro/consumo de receitas.
...
| PB-37 | Como time tecnico, quero arquitetura modular por features com injecao de dependencia e fluxo reativo para facilitar evolucao e manutencao. | Highest | 8 |
...
### Sprint 2 - Planejamento, compras e offline/tempo real (85 pontos)

Objetivo da sprint:
- Entregar planejamento semanal completo, colaboracao de compras e robustez de uso online/offline.
...
### Sprint 3 - Inteligencia, experiencia de preparo e comunidade (117 pontos)

Objetivo da sprint:
- Concluir os diferenciais de inteligencia, voz, social, acessibilidade e qualidade final de entrega.
...
- Historia implementada com criterios de aceite atendidos.
- Build e lint sem erro bloqueante.
- Teste manual do fluxo principal concluido.
- Pull Request vinculada a issue PB-XX com descricao do que foi entregue.
- Evidencia da entrega registrada em pasta de evidencias (print, video curto ou log).
- Descricao da issue atualizada com decisoes tecnicas, riscos e limitacoes conhecidas.

## Processo operacional no GitHub (checklist)

1. Criar issue por historia PB-XX usando o titulo padrao PB-XX - resumo curto.
2. Aplicar labels minimas: epic:..., priority:..., points:..., type:story.
3. Associar a issue ao milestone da sprint correta.
4. Vincular PR a issue com fechamento automatico (exemplo: closes #123).
5. Ao concluir, anexar evidencia em docs/evidencias e marcar issue como Done no Project.

## Evidencias por sprint (sugestao)

- Criar pasta `docs/evidencias/sprint-1`, `docs/evidencias/sprint-2`, `docs/evidencias/sprint-3`.
- Para cada historia finalizada, salvar ao menos 1 evidencia nomeada como `PB-XX-descricao-curta.png` ou `.mp4`.
- Manter um arquivo `docs/evidencias/README.md` com links para cada evidencia e data de validacao.
